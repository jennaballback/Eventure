...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($title) || empty($start_time)) {
        ...
        $stmt = $conn->prepare("INSERT INTO events (host_id, title, description, location, start_time, end_time, theme, is_canceled) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("issssssi", $host_id, $title, $description, $location, $start_time, $end_time, $theme, $is_canceled);
